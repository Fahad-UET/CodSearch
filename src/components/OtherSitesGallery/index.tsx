...
interface OtherSitesGalleryProps {
  product: Product;
...
export function OtherSitesGallery({
  product,
  onUpdateProduct,
  onClose,
  embedded = false,
}: OtherSitesGalleryProps) {
  ...
  const handleDeleteLink = async (linkId: string) => {
    try {
      const links = (product.otherSite || []).filter(link => link.id !== linkId);
      const updatedProduct = await updateProductService(product.id, { otherSite: links });
      if (!updatedProduct) {
        throw new Error('Failed to update product');
      }
  ...
  const content = (
    <div className="p-6 space-y-6 h-[70vh]">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {(!product.otherSite ||
          product.otherSite.filter(link => link.type === 'other').length === 0) && (
          <div className="text-center py-12 text-gray-500">No links added yet</div>
        )}
      </div>
      <Notification
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-purple-900/95 via-indigo-900/95 to-blue-900/95 backdrop-blur-md z-[200] flex items-center justify-center">
      <div className="relative w-full max-w-7xl h-[90vh] mx-4 bg-gradient-to-br from-white via-purple-50 to-indigo-50 rounded-2xl overflow-hidden shadow-2xl border border-white/20">
        <div className="flex justify-between items-center p-6 border-b border-purple-100">
          <h2 className="text-xl font-semibold text-gray-900">Other Sites</h2>
