...
export function TemplateManager({ onSelectTemplate }: TemplateManagerProps) {
  ...
  const { variables: globalVariables, replaceVariables } = useVariableStore();
  ...
  const handleEditVariable = (id: string, updates: Partial<TemplateVariable>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Variables Panel */}
      {showVariables && (
        <div className="bg-white rounded-xl border border-purple-100 p-4 shadow-lg">
          <VariablesList onSelectVariable={(variable) => {
            const textarea = textareaRef.current;
            if (!textarea) return;
