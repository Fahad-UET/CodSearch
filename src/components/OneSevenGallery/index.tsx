...
interface OneSevenGalleryProps {
  product: Product;
...
export function OneSevenGallery({
  product,
  onUpdateProduct,
  onClose,
  embedded = false,
}: OneSevenGalleryProps) {
  ...
  const handleDeleteLink = async (linkId: string) => {
    try {
      const links = (product.oneSix || []).filter(link => link.id !== linkId);
      const updatedProduct = await updateProductService(product.id, { oneSix: links });
      if (!updatedProduct) {
        throw new Error('Failed to update product');
      }
  ...
  const mainContent = (
    <div className="p-6 space-y-6 h-[70vh]">
      <div className="bg-white rounded-xl p-6 border border-purple-100 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <MarketplacePriceInput
            currentPrice={product.competitorPrices?.oneSeven || 0}
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder="Enter 1688.com URL"
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {(!product.oneSix ||
          product.oneSix.filter(link => link.type === 'oneSix').length === 0) && (
          <div className="text-center py-12 text-gray-500">No 1688.com links added yet</div>
        )}
      </div>
      <Notification
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-purple-900/95 via-indigo-900/95 to-blue-900/95 backdrop-blur-md z-[200] flex items-center justify-center">
      <div className="relative w-full max-w-7xl h-[90vh] mx-4 bg-gradient-to-br from-white via-purple-50 to-indigo-50 rounded-2xl overflow-hidden shadow-2xl border border-white/20">
        <div className="flex justify-between items-center p-6 border-b border-purple-100">
          <h2 className="text-xl font-semibold text-gray-900">1688.com Links</h2>
          <button
            onClick={onClose}
