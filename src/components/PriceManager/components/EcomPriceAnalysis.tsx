...
import { shippingCosts } from '@/components/ProfitabilityCalculator/constants';
import { get } from 'http';

interface EcomPriceAnalysisProps {
  salePrice: number;
  ...
  setNetProfit?: any;
  setProfitPerProdcut: any;
  product: any;
  purchasePrice: number;
  getCurrencyLogo: any;
  setProfit?: any;
  // requiredLeads:number |string
}
...
export function EcomPriceAnalysis({
  salePrice,
  sourcingPrice,
  settings,
  countrySettings,
  ...
  const metrics = useMemo(() => {
    ...
    const costBreakdowns: Record<string, CostBreakdown> = {
      ...
      shippingCosts: {
        ...
        details: [
          { label: 'Required Leads', value: String(requiredLeads) },
  ...
  const cards = [
    {
      ...
      icon: DollarIcon,
      color: 'purple',
      value: salePrice ? metrics.totalSales : 0, // Condition added here
      formula: `Total Sales = Sale Price × Stock\n${salePrice.toFixed(2)} × ${Math.round(
        settings.stock
      )} = ${salePrice ? Math.round(metrics.totalSales) : 0}`,
      details: [
        {
          label: 'Sale Price',
          value: salePrice ? `${Math.round(salePrice)} ${getCurrencyLogo}` : '$0',
        }, // Condition added here
        { label: 'Stock', value: settings.stock },
        {
          label: 'Total Sales',
          value: salePrice ? `${Math.round(metrics.totalSales)} ${getCurrencyLogo}` : '$0',
        }, // Condition added here
      ],
    },
    {
      ...
      icon: BuildingIcon,
      color: 'red',
      value: salePrice ? metrics.totalCostsWithFullfilment : 0, // Ensure it defaults to 0 if totalCosts is undefined
      formula: `Total Costs = Product Cost + Shipping Cost + Returns Cost + Call Center Cost + Fulfillment Cost \n ${
        metrics?.productCosts
      } + ${metrics?.shippingCostUpdated} + ${metrics?.getTotalReturnCost} +  ${
        metrics?.totalCallCenter || 0
      } + ${metrics?.fullfilmentCost} = ${metrics?.totalCostsWithFullfilment}`,
    ...
    },
    {
      ...
      icon: MegaphoneIcon,
      color: 'purple',
      value: salePrice && metrics.advertisingCost, // Ensure it defaults to 0 if advertisingCost is undefined
      formula: `Advertising Cost = Required Leads × Cost Per Lead (CPL)\n${
        metrics.requiredLeads || 0
      } × ${settings.cpl || 0} = ${Math.round(metrics.advertisingCost) || 0}`,
      details: [
        { label: 'Required Leads', value: metrics.requiredLeads || 0 }, // Default to 0 if requiredLeads is undefined
        { label: 'Cost Per Lead (CPL)', value: `${settings.cpl || 0} ${getCurrencyLogo}` }, // Default to 0 if CPL is undefined
        {
          label: 'Advertising Cost',
          value: `${Math.round(metrics.advertisingCost) || 0} ${getCurrencyLogo}`,
        }, // Default to 0 if advertisingCost is undefined
      ],
    },
    {
      id: 'grossProfit',
      ...
      icon: WalletIcon,
      color: 'green',
      value: salePrice && metrics.netProfit, // Default to 0 if netProfit is undefined
      formula: `Net Profit = Gross Profit - Operating Expense\n${Math.round(
        metrics.grossProfit || 0
      )} - ${metrics.operatingExpense?.toFixed(2) || 0} = ${Math.round(metrics.netProfit || 0)}`,
    ...
    <div className="space-y-4">
      {cards.map(card => (
        <div>
          <div
            key={card.id}
            className={`w-full relative bg-gradient-to-br from-white to-gray-50/95 rounded-xl shadow-lg border-2 border-indigo-100 hover:border-purple-200 transition-all duration-200 group
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex justify-between items-center gap-1">
                  <div className="text-xl font-bold bg-gradient-to-r from-violet-600 to-indigo-600 bg-clip-text text-transparent group-hover:scale-105 transition-transform duration-200">
                    {Math.round(card.value)} {getCurrencyLogo}
                  </div>
                              ...
                              >
                                ...
                                <div className="flex items-center gap-3">
                                  <span className="text-sm font-medium text-purple-600">
                                    {Math.round(detail.value)} {getCurrencyLogo}
                                  </span>
