  ...
  const cards = [
    {
      ...
      icon: DollarIcon,
      color: 'purple',
      value: salePrice && sourcingPrice ? metrics.totalSales : 0, // Condition added here
      formula: `Total Sales = Sale Price × Stock\n${salePrice.toFixed(2)} × ${Math.round(
        settings.stock
      )} = ${salePrice && sourcingPrice ? Math.round(metrics.totalSales) : 0}`,
      details: [
        { label: 'Sale Price', value: salePrice ? `$${Math.round(salePrice)}` : '$0' }, // Condition added here
        { label: 'Stock', value: settings.stock },
        {
          label: 'Total Sales',
          value: salePrice && sourcingPrice ? `$${Math.round(metrics.totalSales)}` : '$0',
    ...
    },
    {
      ...
      icon: BuildingIcon,
      color: 'red',
      value: salePrice && sourcingPrice ? metrics.totalCosts : 0, // Ensure it defaults to 0 if totalCosts is undefined
    ...
    },
    {
      ...
      icon: MegaphoneIcon,
      color: 'purple',
      value: salePrice && sourcingPrice ? metrics.advertisingCost : 0, // Ensure it defaults to 0 if advertisingCost is undefined
      ...
      details: [
        ...
        { label: 'Cost Per Lead (CPL)', value: `$${settings.cpl || 0}` }, // Default to 0 if CPL is undefined
        { label: 'Advertising Cost', value: `$${Math.round(metrics.advertisingCost) || 0}` }, // Default to 0 if advertisingCost is undefined
      ],
    },
    {
      id: 'grossProfit',
      ...
      icon: WalletIcon,
      color: 'green',
      value: salePrice && sourcingPrice ? metrics.netProfit : 0, // Default to 0 if netProfit is undefined
