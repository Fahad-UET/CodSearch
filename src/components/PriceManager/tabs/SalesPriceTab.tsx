...
const SettingIcon = ({ onclick }: settingsProps) => {
  return (
    <div
...
export function SalesPriceTab({
  ...
  const [selectedCountry, setSelectedCountry] = useState(product?.country || 'KSA');
  ...
  const [settings, setSettings] = useState({
    ...
    fullFillment: product?.fullFillment || 10,
  });
  // country conversion
  ...
  const metrics = useMemo(() => {
    ...
    const costBreakdowns: Record<string, any> = {
      ...
      returnCosts: {
        ...
        details: [
          ...
          { label: 'Shipping Cost Returned', value: `$${shippingCosts.return}` },
    ...
    return {
      ...
      getTotalReturnCost,
      productCosts,
      shippingCost: shippingCostUpdated,
      totalCallCenter,
      codFee,
      codFees,
      profitPerProduct,
  ...
  }, [settings, countrySettings, salePrice, sourcingPrice, product]);
  const handleSaveSalePrice = async () => {
    ...
    try {
      ...
      const kpiValues = {
        ...
        callCenterFees: metrics?.callCenterFees,
        codFee: metrics?.codFee,
        profit,
        ...
        <div className="flex items-center gap-4">
          <div className="w-1/4 relative">
            <input
              ...
              placeholder="0.00"
            />
            <p className="font-medium text-lg text-indigo-600 absolute right-2 top-2.5">
              {countryListCurrency.currency}
            </p>
          </div>
          <div className="flex-1 flex items-center gap-2 overflow-x-auto py-1">
            {getDefaultCountryValues?.map((item, ind) => (
        ...
        <div className="w-[60%] ">
          <div className="w-full flex justify-between items-stretch gap-4">
            ...
            <div className="bg-gradient-to-br w-full from-violet-500 to-purple-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              <h3 className="text-sm font-medium opacity-80 mb-2">Sales Price</h3>
              <div>
            ...
            <div className="bg-gradient-to-br w-full from-indigo-500 to-blue-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              <h3 className="text-sm font-medium opacity-80 mb-2">Sourcing Price</h3>
              <div className="text-2xl font-medium mb-3">
                PSP: ${`${sourcingPrice.toFixed(2) || 0}`}
              </div>
              <div className="flex justify-between gap-2">
                <button
                  onClick={() => setShowCalculator(true)}
                  className="flex items-center space-x-2 bg-white/10 rounded-lg px-3 py-1.5 text-sm hover:bg-white/20"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                ...
                <input
                  ...
                  step="0.01"
                  value={
                    sourcingPrice !== null && sourcingPrice !== undefined ? sourcingPrice : '0.00'
                  }
                  onChange={e => {
                    const value = parseFloat(e.target.value);
                    if (!isNaN(value) && value >= 0) {
                      setSourcingPrice(value);
                    } else {
                      setSourcingPrice(0); // Handle invalid input or reset to 0
                    }
                  }}
                  className="w-full pl-2 h-12 outline-none text-xl font-medium bg-white/10 border-white/20 rounded-[8px] text-white placeholder-white/50 focus:border-white focus:ring-2 focus:ring-white/20"
                  placeholder="0.00"
                />
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br w-full from-teal-500 to-emerald-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              ...
              <div className="text-2xl font-medium">
                PPD: {metrics?.profitPerProduct.toFixed(2) || 0}
              </div>
              <div className="flex items-center space-x-2 mt-2">
                ...
                <span className="text-sm">
                  PM {salePrice && sourcingPrice ? metrics?.getProfitMargin.toFixed(2) : 0}%
                </span>
              </div>
              <div className="flex items-center space-x-2 mt-2">
                <svg
                ...
                <span className="text-sm">
                  ROI {salePrice && sourcingPrice ? getCalculateRoi.toFixed(1) : 0}%
                </span>
              </div>
            </div>
          </div>
