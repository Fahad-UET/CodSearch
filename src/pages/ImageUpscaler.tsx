...
function ImageUpscaler() {
  ...
  const [upscalingFactor, setUpscalingFactor] = React.useState<UpscalingFactorEnum>('4');
  ...
  const handleHistoryItemClick = item => {
    if (item.type === 'image-upscaler' && item.content.imageUrl) {
  ...
  React.useEffect(() => {
    const handleResponse = ((event: CustomEvent) => {
      if (event.detail.type === 'image-upscaler' && event.detail?.images) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestOptions = {
        image_url: imageUrl,
        upscaling_factor: parseInt(upscalingFactor),
      };
      ...
      // Submit request
      const result: any = await submitToQueue<UpscalerResponse>(
      ...
      controls={
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-white/60">Upload an image to upscale (max 10MB)</p>
          </div>

          <div className="space-y-2">
            <label className="block text-sm font-medium">Upscaling Factor</label>
            <select
              value={upscalingFactor}
              onChange={e => setUpscalingFactor(e.target.value as UpscalingFactorEnum)}
              className="input-area"
            >
              <option value="4">4x</option>
            </select>
            <p className="text-sm text-white/60">Increase resolution by 4x</p>
          </div>
      ...
      result={
        <>
          ...
          {loading && !outputUrl && (
            <LoadingAnimation
              progress={percentage}
              progressText={progress || 'Upscaling image...'}
          ...
          {(outputUrl || id) && (
            <div className="mt-8 space-y-4">
              <ImageContainer outputUrl={outputUrl} title={'Upscaled Image'} imageLoading={imageLoading} />
            </div>
          )}
        </>
      }
