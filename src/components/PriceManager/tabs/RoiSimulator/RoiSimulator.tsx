...
import { RoiSimulatorAnaylsis } from './RoiSimulatorAnalysis';
// import { createDefaultCountryValues } from '@/services/firebase/deafultCountries';
import { CostBreakdown } from './RoiSimulatorAnalysis';
import {
...
const checkTargetProductType = {
  'Per Lead': 'Lead Target',
  'Per Deliverd': 'Target Product Delivered',
  'Per Confirmed': 'Target Product Confirmed',
};
...
export function RoiSimulator({
  ...
  const [selectTargetProductType, setselectTargetProductType] = useState<string>('Per Lead');
  ...
  // const [comission, setComission] = useState<number | string>(settings?.monthlyProductCharges || 0);
  const [comission, setComission] = useState<number>(23);
  // country conversion
  ...
  useEffect(() => {
    setSettings(prev => ({ ...prev, cpl: usdCurrency }));
  }, [usdCurrency]);
  ...
  const metrics = useMemo(() => {
    const requiredLeads =
      defaultValueComissionType === 'Per Deliverd'
        ...
        : defaultValueComissionType === 'Per Confirmed'
        ? Math.ceil(settings?.stock / (settings.confirmationRate / 100))
        : Math.ceil(settings.stock);
  ...
  }, [settings, countrySettings, salePrice, sourcingPrice, comission, defaultValueComissionType]);
  ...
  const handleSaveSalePrice = async () => {
    ...
    try {
      setIsSaving(true);
      const totalExpenses = metrics.advertisingCost;
      ...
      const kpiValues = {
        ...
        stockInvestment: 30,
        costPerDelivered,
        requiredLeads: metrics.requiredLeads,
        totalSales: {
        ...
        <div className="flex items-center gap-4">
          <div className="w-1/4 relative">
            <input
              ...
              className="w-full px-4 py-2.5 text-xl font-medium bg-white/90 border-2 border-indigo-200/50 rounded-lg focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200 focus:ring-opacity-50"
              placeholder="0.00"
            />
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-purple-200/30">
            <div className="flex items-center justify-between text-sm mb-2">
              ...
              <span className="text-xl font-bold text-purple-600">
                {/* {requiredLeads} */}
                {Math.floor(metrics?.requiredLeads) || 0}
              </span>
            </div>
        ...
        <div className="w-[60%] ">
          <div className="w-full flex justify-between items-stretch gap-4">
            ...
            <div className="bg-gradient-to-br w-full from-violet-500 to-purple-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              <h3 className="text-sm font-medium opacity-80 mb-2">Sale Price</h3>
              <div className="text-2xl font-medium mb-1">SP: ${salePrice.toFixed(2)}</div>
              <div>
            ...
            <div className="bg-gradient-to-br w-full from-indigo-500 to-blue-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              <h3 className="text-sm font-medium opacity-80 mb-2">Comission </h3>
              <div className="mb-2">
                <h3 className="text-sm font-medium  mb-1">Comission Type</h3>
                <select
                  value={defaultValueComissionType}
                  onChange={e => {
                    setSettings(prev => ({
                      ...prev,
                    ...
                    setselectTargetProductType(e.target.value);
                    setdefaultValueComissionType(e.target.value);
                  }}
                  className="w-full pl-1 h-12 outline-none text-xl font-medium bg-white/10 border-2 border-white/20 rounded-[8px] text-white placeholder-white/50 focus:border-white focus:ring-2 focus:ring-white/20"
                >
                  <option className="text-gray-600" value="Per Deliverd">
                    Per Deliverd
                  </option>
                  <option className="text-gray-600" value="Per Lead">
                    Per Lead
                  </option>
                  <option className="text-gray-600" value="Per Confirmed">
                    Per Confirmed
                  </option>
                </select>
              </div>
              <div>
                <h3 className="text-sm font-medium  mb-1">Comission</h3>
                <div className="relative">
                  <input
                    type="number"
                    min="0"
                    ...
                    className="w-full pl-7 h-12 outline-none text-xl font-medium bg-white/10 border-2 border-white/20 rounded-[8px] text-white placeholder-white/50 focus:border-white focus:ring-2 focus:ring-white/20"
                    placeholder="0.00"
                  />
            ...
            <div className="bg-gradient-to-br w-full from-teal-500 to-emerald-600 rounded-xl p-4 text-white shadow-lg border border-white/20">
              ...
              <div className="flex items-center space-x-2 mt-2">
                ...
                <span className="text-sm">
                  PM {salePrice && comission ? metrics?.getProfitMargin.toFixed(2) : 0}%
                </span>
              </div>
              <div className="flex items-center space-x-2 mt-2">
                <svg
                ...
                <span className="text-sm">
                  ROI {salePrice && comission ? getCalculateRoi.toFixed(1) : 0}%
                </span>
              </div>
            </div>
          </div>
          ...
          />
          <RoiSimulatorAnaylsis
            grosstitleBase={checkTargetProductType[selectTargetProductType]}
            comission={comission}
            salePrice={salePrice}
            sourcingPrice={sourcingPrice}
            settings={settings}
